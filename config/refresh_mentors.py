...
def get_airtable_records():
    ...
    try:
        ...
        logger.info(f"Successfully fetched {len(records)} mentor records from Airtable")
        return records
...
def convert_records_to_dataframe(records):
    ...
    logger.info(f"Converted {len(df)} mentor records to DataFrame")
    return df
...
def update_mentors_csv(df, output_path=None):
    ...
    logger.info(f"Total mentors: {len(df)}")
    ...
    if 'dates' in df.columns:
        ...
        logger.info(f"Available dates: {sorted(list(all_dates))}")
